...
##                                                    s   s   s   s   s
##                                                    _   _   _   _   _
##   Our_sample sex weight_kg    popn dropheight_m    1   2   3   4   5
## 1   sample_1   M     112.3     Tas            3   AB AAO AAO AAO  AB
## 2   sample_2   F     125.7     Tas            4  AAO  AB  AB AAO  AB
## 3   sample_3 Imm      96.6 Eastern            2  AAO AAO AAO AAO  AB
```
...
```
##         wpsex   i   j nABOO
## 92 0.05321628 372 371     0
## 93 0.05418490 296 295     0
## 94 0.05478877 224 223     0
## 95 0.05495167 210 209     0
## 96 0.05498302 316 315     0
```
...
POPs$ij <- upairid(POPs$i, POPs$j) ## a pair ID
```
...
POPsOrFSPs$ij <- upairid(POPsOrFSPs$i, POPsOrFSPs$j) ## pair ID to match the one in POPs
...
large) proportion of true 2KPs, subject to a tolerance on the expected
number of contaminating 3KPs and 4KPs that by chance fall above the
threshold. That tolerance is up to you; you really don't want many
false-positives from 3KPs because they can mess up a CK model. The
problem isn't the _number_ of false-positives; rather, it's the
different dynamics that different kinships have. Most of all, 3KPs
tend to have much longer birth-gaps than HSPs, so that mislabelling
even a couple of 3KPs as "HSPs" can cause a substantial bias in
mortality-rate estimates. (Note that it doesn't cause bias to discard
some true 2KPs below the cutoff, because we can compensate by
incorporating the estimated false-negative rate directly into the CKMR
model.) While it's a good idea to choose a high cutoff (i.e., few
false-positives) to minimise the bias from 3KPs, there is a trade-off;
higher cutoffs do lead to fewer definite 2KPs going into the model,
and thus to higher variances, as well as making it harder to check
model diagnostics. Nevertheless, you should certainly go high enough
to get your estimated false-positive rate well _below_ the
false-negative rate; you defintely _don't_ want "equal probabilities"
here.

We have often chosen the cutoff at a round number by eye, but a more
systematic approach is via the `autopick_threshold` function. This
takes the empirical distribution of PLODs in the tentative HSPs,
estimates the impact of linkage, and--- on the assumption that each
comes from either a HSP, HTP, or half first cousin pair (H1CP)--- fits
a constrained mixture decomposition (see function documentation and MS
for details) to estimate the underlying distributions and numbers of
HSPs/HTPs/H1CPs, before numerically choosing a cutoff to achieve the
desired tolerance.^[note that we're using 'HSP', 'HTP', and 'H1CP' in
this assumption, rather than '2KP', '3KP', and '4KP'. The distinction
arises because, although expected PLODs are the same within each of
2KP, 3KP, and 4KP, linkage means that PLOD _variances_ can differ
between kin-types within a kinship order, and 'autopick_threshold'
assumes HSPs, HTPs, and H1CPs specifically. This is unlikely to have
much impact in practice, but is one more reason to use demographic
information to minimise the number of non-HSP 2KPs at this stage of
the analysis] When `autopick_threshold` works well, it's great; but
don't trust its output if it doesn't look sensible, because it may
have too much flexibility in fitting especially when the number of
kin-pairs is low. Whether-or-not you use `autopick_threshold`, the
process _is_ intrinsically subjective, because _you_ have to decide
how many 3KPs you are willing to tolerate. So, picking a cutoff by eye
is also OK (because the false-negative rate computed afterwards will
eliminate bias), as long as you don't try to "tweak" the cutoff to
include/exclude specific pairs.



``` r
HSPs_1$ij <- upairid(HSPs_1$i, HSPs_1$j)
...
## 179.66091016 192.07090954 162.47650979 166.37340040 135.90524760 185.86912547 
##        C_hat      rho_hat           E2           E3           E4          SD2 
## 211.68747051   0.05300852  61.60599802  -1.43495747 -32.95543522  13.63338276 
##          SD3          SD4           P2           P3           P4           N3 
##  13.85896495  12.89858133   0.14357251   0.13856494   0.71786255  49.27040441 
##         Nall       thresh      Pr_FNeg 
## 355.57627543  26.94363293   0.00550372
```
...
```
##      Our_sample         sex        weight_kg             popn    
##  Length   :104   Length   :104   Min.   : 47.30   Length   :104  
##  N.unique :104   N.unique :  4   1st Qu.: 84.42   N.unique :  3  
##  N.blank  :  0   N.blank  :  0   Median :101.30   N.blank  :  0  
##  Min.nchar: 10   Min.nchar:  1   Mean   : 98.88   Min.nchar:  3  
##  Max.nchar: 10   Max.nchar:  3   3rd Qu.:116.40   Max.nchar:  7  
##                                  Max.   :138.50                  
##   dropheight_m   
##  Min.   : 0.000  
##  1st Qu.: 2.000  
...
```
##      Our_sample         sex        weight_kg             popn    
##  Length   :480   Length   :480   Min.   : 29.00   Length   :480  
##  N.unique :480   N.unique :  4   1st Qu.: 88.72   N.unique :  3  
##  N.blank  :  0   N.blank  :  0   Median :107.85   N.blank  :  0  
##  Min.nchar:  8   Min.nchar:  1   Mean   :102.02   Min.nchar:  3  
##  Max.nchar: 10   Max.nchar:  3   3rd Qu.:119.03   Max.nchar:  7  
##                                  Max.   :161.10                  
##   dropheight_m   
##  Min.   : 0.000  
##  1st Qu.: 2.000  
...
## Restricting comparisons with sample metadata

The `subset1` and `subset2` arguments select **rows of the supplied
`snpgeno`**, not sample names or positions within another subset. Use
positive numeric indices, for example those returned by `which()`.
The two subsets must be identical or disjoint; partially overlapping
subsets are not supported. Identical subsets compare each distinct pair
once, without self-comparisons. Disjoint subsets compare across the two
groups only.

For illustration, the following compares samples labelled `Imm` with
those labelled `M` or `F` in the example data. These labels are specific
to this dataset; use validated age/maturity metadata in a real study.
This is a demonstration of restricting comparisons, not a recommendation
to exclude other HSP comparisons.

```{r subset-comparisons}
subset_data <- kinference::kin_power(kinference::dropbears, k = 0.5)
immature <- which(subset_data$info$sex == "Imm")
adult_label <- which(subset_data$info$sex %in% c("M", "F"))
stopifnot(length(immature) > 0L, length(adult_label) > 0L,
          length(intersect(immature, adult_label)) == 0L)
subset_hsps <- kinference::find_HSPs(
  subset_data, subset1 = immature, subset2 = adult_label,
  keep_thresh = 0, limit_pairs = 1000, ij_numeric = TRUE
)
# Returned i and j refer to the ORIGINAL rows in subset_data.
subset_hsps$sample1 <- subset_data$info$Our_sample[subset_hsps$i]
subset_hsps$sample2 <- subset_data$info$Our_sample[subset_hsps$j]
stopifnot(all(subset_hsps$i %in% immature),
          all(subset_hsps$j %in% adult_label))
head(subset_hsps)
```

Subset selection changes which pairs are searched. It does **not**
re-estimate allele frequencies for each group, adjust population
structure, or correct plate/batch effects. Those assumptions and data
quality checks still need attention before interpreting relationships.
The `limit_pairs` cap may truncate retained candidates; inspect any
truncation message and increase it where appropriate.
...
we'll use the `bluefin` dataset included in package `kinference`. This is a small, anonymised subset of the tuna close-kin data described in Farley et al., 2024; see `?bluefin` for details and full citation.
...
``` r
# 'bluefin' is a dataset that comes with 'kinference'; see '?bluefin'
