...
#  use4 <- !use6
  ...
  bits <- result %without.name% cq( big_PLOD, big_i, big_j)
  result <- with( result, data.frame( PLOD=big_PLOD, i=big_i, j=big_j))
  attributes( result) <- c( attributes( result), bits)
...
stopifnot( my.all.equal( subset1, subset2) || !length( intersect( subset1, subset2)))
  ...
  define_genotypes()
  extract.named( snpg@locinfo[ cq( use6, pbonzer)])
  p0 <- pbonzer[,'O'] + pbonzer[,'C']
  pA <- pbonzer[,'A']
  pB <- pbonzer[,'B']
...
stopifnot( all( ww>0))
  ...
  temp_snpg <- snpg[ , pop_loci]
  # Decide what to treat as a PSEXCLU later
  if( my.all.equal( temp_snpg$diplos, genotypes4_ambig)) {
    AAish <- AAO
    BBish <- BBO
  } else if( my.all.equal( temp_snpg$diplos, genotypes6)) { # normally...
    recode4to6temp <- function( x) { x[ x=='AO'] <- AA; x[ x=='BO'] <- BB; x}
    temp_snpg <- recode4to6temp( temp_snpg) # (AA,AO) -> AA; (BB,BO) -> BB
    AAish <- AA
    BBish <- BB
  } else {
stop( "find_POPs needs 4way or 6way ABO-genotypes")
  }
  ...
  # These for expected values
  pex <- within( data.frame( pex), {
    HSP <- 0.5*UP + 0.5*POP
    FSP <- 0.25*UP + 0.5*POP
  })
  ...
  n_sim_check <- 1000
  Ktest <- function( tt) {
    x <- matrix( runif( n_sim_check * n_loci) < pex$UP, n_loci, n_sim_check)
    ewx <- exp( x*ww*tt)
    colSums( log( ewx))
  }
  ...
  symmo <- my.all.equal( subset1, subset2)
  inv_CDF <- renorm_SPA_cumul( K, dK, ddK, already_vectorized=TRUE)$inv_CDF
  ...
  if( symmo){
    ...
    result <- POP_wt_paircomps_lots(
        ...
        max_keep_wpsex= keep_thresh,
        bins= pciles,
        AAO= match( AAish, snpg@diplos), # NB NB: AO has been recoded to AA
        BBO= match( BBish, snpg@diplos)
      )
  } else { # different subsets
    result <- POP_wt_paircomps_lots(
        geno1= temp_snpg[ ,subset1],
  ...
  # construct the result
  n_in_bin <- result$n_wpsex_in_bin
  result <- with( result, data.frame( wpsex=big_wpsex, i=big_i, j=big_j))
  ...
  result@E_POP <- ww %**% pex$POP
  result@E_FSP <- ww %**% pex$FSP
  result@E_HSP <- ww %**% pex$HSP
