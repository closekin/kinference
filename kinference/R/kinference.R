...
#' from \code{geno_deambig_ABC}. NALF (null allele frequency) is estimated from
#' HWE deviations, so this requires a decent sample size. But, it doesn't
...
#' @seealso \code{\link{est_ALF_ABCO}}
#' @keywords misc
#' @examples
#'
#' # Should have one
...
#' FAULT]: \item{ i j::}{} \item{ 3 1::}{} \item{ 4 3::}{} \item{ 6 4}{but you
#' won't see the pairings for 1/4, 1/6, 3/6. If you just want to strip out all
#' duplicates bar one in each group (and you don't care which one is kept),
...
function(snpg, hist_pars=list(), showPlot = TRUE) {
  ...
  if( quick) {
    lpgeno <- lpgeno3
    K3 <- compile_vecless( K( -1))
    dK3 <- compile_vecless( dK( -1))
    ddK3 <- compile_vecless( ddK( -1))
    lpgeno <- lpgeno4
    K4 <- compile_vecless( K( -1))
    dK4 <- compile_vecless( dK( -1))
    ddK4 <- compile_vecless( ddK( -1))
  }
  ...
  if(any( snpg@locinfo$useN == 3)) {
      ilglk3 <- indiv_lglk_geno(
      lpgeno= lpgeno3[ snpg@locinfo$useN == 3,,  drop = FALSE],
      geno= snpg4[ , snpg@locinfo$useN == 3, drop = FALSE]) ## calc for all individuals, at loci where useN == 3
  }
  if( any( snpg@locinfo$useN > 3)) {
      ilglk4 <- indiv_lglk_geno(
      lpgeno= lpgeno4[ snpg@locinfo$useN > 3,, drop  =FALSE],
      geno= snpg4[ , snpg@locinfo$useN > 3, drop = FALSE])
  }
  if( !exists('ilglk3')) { ilglk3 <- rep(0, nrow(snpg)) }
  if( !exists('ilglk4')) { ilglk4 <- rep(0, nrow(snpg)) }

  ilglk <- rowSums(cbind( ilglk3, ilglk4), na.rm = TRUE)
  ...
  dens_SPA <- renorm_SPA( K4, dK4, ddK4, return='func', already_vectorized=TRUE)
...
#'
#' \describe{ \item{pick between them with }{this doesn't work very well yet...
#' use age info if you can)}\item{list("split_FSPs_from_POPs")}{this doesn't
#' work very well yet... use age info if you can)}\item{ (update}{this doesn't
#' work very well yet... use age info if you can)} }
#'
