...
#' 
#' \itemize{ \item QC of samples and loci, with the specific goal of then being
#' able to... \item find close-kin pairs and duplicate samples. }
#' 
...
#' rates should be low (so, no "3X coverage" etc!). Null alleles[1] are allowed
...
#' 
#' [1] "Null" here means: heritable and repeatable undetectable allele, eg due
#' to mutations at restriction site or indels nearby. It specifically excludes
...
#' \subsection{Locus qc\code{check_6and4} (qv) \subsection{Sample
#' qc\code{\link{ilglk_geno}} (qv), \code{\link{hetzminoo_fancy}} (qv),
#' \code{\link{find_duplicates}} (qv), \code{\link{find_dups_with_missing}}
#' (qv), \code{\link{drop_dups_pairwise_equiv}} (qv) } \subsection{Allele
#' frequency estimation\code{\link{est_ALF_ABO_quick}} (qv), or less likely
#' \code{\link{est_ALF_6way}} (qv), \code{\link{est_ALF_ABCO}} (qv),
#' \code{\link{re_est_ALF}} (qv) } \subsection{Pairwise kin
#' finding\code{\link{find_POPs}} (qv), \code{\link{find_POPs_lglk}} (qv) when
#' it's working properly (not yet), \code{\link{find_HSPs}} (qv),
#' \code{\link{histoPLOD}} for graphics, \code{\link{kinPalette}} ditto,
#' \code{\link{split_FSPs_from_HSPs}} (qv) and other \code{split_X_from_Y}
#' functions, \code{\link{autopick_threshold}} (qv), \code{\link{var_PLOD_kin}}
#' (qv) } \subsection{Predicting locus power for kin
#' finding\code{\link{kin_power}} (qv) } \subsection{Example
#' datasets\code{\link{dropbears}}, \code{\link{bluefin}} } }
#' @keywords misc
NULL
