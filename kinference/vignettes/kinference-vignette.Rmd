...
Package `kinference` uses SNP genotype data to find pairs of
...
## 1   sample_1         1   ?     113.0 Eastern            6   AB  AA  AA  AA  AB
## 2   sample_2         1   M     112.7 Central            4   AA  AB  AB  AA  AB
## 3   sample_3         1   M      98.8 Eastern            5   AA  AA  AA  AA  AB
```
...
eg POP vs FSP, or 2OP VS UP. But here we have at least 4 possible
...
```
##  [1] "names"          "row.names"      "class"          "bins"           "n_wpsex_in_bin"
##  [6] "eta"            "keep_thresh"    "n_loci"         "mean_UP"        "var_UP"        
## [11] "call"
```
...
FSPs^[There might also be some 2OPs within `strongest`, but in terms
...
```r
## the clump between -50 and -100 is POPs; the rest are a mix of FSPs, HSPs, and possibly weaker
## kin.
...
by provisionally identifying "all interesting kin" (1OPs and 2OPs, but
perhaps with some 3OPs etc mixed in), taking out the POPs that we
...
`find_HSPs`--- which should really be called `find_2OPs`--- keeps
...
happens that the expected PLOD for an 2OP is roughly the absolute
value of the UP mean, and for a 3OP the expected PLOD is close
to 0. If kinference is going to work, the 2OP bump should be well to
the right of the 3OP mean, so choosing `keep_thresh` just below 0 (ie
...
variance exactly where theory predicts them to be, and bumps for 2OPs
and 1OPs (ie, FSP as well as POP) centred exactly where theory
expects^[Recall that POP stats were slightly off. However, 2OP _stats_
seem a bit more robust, even though 2OPs themselves are harder to
identify. So if these HSP/2OP stats _aren't_ very well aligned in your
data, be nervous.]. There's also a "shoulder" around 0, between the UP
and 2OP bumps, which is just where we'd expect 3OPs to be. This is a
good dataset.
...
hopefully between 1OPs and 2OPs (it's not vitally important where; see
'?histoPLOD' for more).
...
and POPs, but there is a clear gap between 1OPs and 2OPs. Any 1OPS
that _aren't_ POPs must be FSPs, so we can pick all 1OPs using this
...
2OPs. Again, those "second-order kin" comprise half-sibling,
...
ages can do the job, and half-siblings will usually be much the
commonest on demographic grounds (details out of scope here). However,
the bigger issue is that the set of tentative 2OPs may well be
contaminated by more-distant kin, mainly 3OP (half-thiatic and
great-grandparent-grandchild pairs) and 4OP (e.g. half-first-cousin
pairs). We have already gotten rid of the 1OPs, but for the rest we
now need to decide on a PLOD cutoff above which we will treat
everything as "almost definite 2OP". We will then use the distribution
of PLODs among those "definite 2OPs" to work out how many
false-negative 2OPs are likely to have fallen below the cutoff. Unlike
with 1OPs vs 2OPs, there won't be a clean break, so things get a bit
more complicated.

The idea of choosing the cutoff is to let us retain a (hopefully
large) proportion of true 2OPs, subject to a tolerance on the expected
number of contaminating 3OPs and 4OPs that by chance fall above the
threshold. That tolerance is up to you; you really don't want many
false-positives from 3OPs because they can mess up a CK model, but the
tighter the tolerance, the fewer definite 2OPs will be left. While
that doesn't bias CK estimates (because we estimate a false-negative
rate to compensate), it does increase the variance; fewer 2OPs also
make it harder to check model diagnostics, and to estimate the
false-negative rate itself. We have often chosen the cutoff at a round
number by eye, but a more systematic approach is via the
`autopick_threshold` function. This takes the empirical distribution
of PLODs in the tentative 2OPs, and--- on the assumption that each
comes from either a 2OP, 3OP, or 4OP--- fits a mixture decomposition
(see function documentation and MS for details) to estimate the
underlying distributions and numbers of 2OPs/3OPs/4OPs, before
numerically choosing a cutoff to achieve the desired tolerance. When
`autopick_threshold` works well, it's great; but don't trust its
output if it doesn't look sensible, because it may have too much
flexibility in fitting especially when the number of kin-pairs is
low. Picking a cutoff by eye is OK (because the false-negative rate
computed afterwards will eliminate bias), as long as you don't try to
"tweak" the cutoff to include/exclude specific pairs.
...
## [1] 1
## attr(,"info")
##        V0_M3        Vx_M3        V0_M4        Vx_M4         V_UP        V_HSP        C_hat 
## 179.73173469 192.16732957 162.50489075 166.41104623 135.87646860 185.98068796 211.29912111 
##      rho_hat           E2           E3           E4          SD2          SD3          SD4 
##   0.05290546  61.59919050  -1.43326485 -32.94949252  13.63747366  13.86244313  12.90004055 
##           P2           P3           P4           N3         Nall       thresh      Pr_FNeg 
##   0.14358149   0.13851105   0.71790746  49.25105851 355.57494499  26.95019383   0.00553123
```
...
## we're missing about 0.5% of HSPs to false-negatives; _extremely_ good, expect 10--20%
## yourself!
...
```
##   Our_sample          Our_plate     sex              weight_kg          popn          
##  Length:104         Min.   :1   Length:104         Min.   : 23.30   Length:104        
##  Class :character   1st Qu.:1   Class :character   1st Qu.: 88.35   Class :character  
##  Mode  :character   Median :1   Mode  :character   Median :109.75   Mode  :character  
##                     Mean   :1                      Mean   :101.98                     
...
```
##   Our_sample          Our_plate     sex              weight_kg         popn          
##  Length:480         Min.   :1   Length:480         Min.   : 10.2   Length:480        
##  Class :character   1st Qu.:1   Class :character   1st Qu.: 86.7   Class :character  
##  Mode  :character   Median :1   Mode  :character   Median :108.2   Mode  :character  
##                     Mean   :1                      Mean   :100.7                     
...
would _expect_ about 0.151 of them to have a P-value less than
1e-4. In practice, they have _always_ looked "worse than expected" on
real data, but often this has not mattered downstream. The P-values
are more to be taken as _relative_ measures of well-behavedness here.
...
```r
## 'thresh...' values are entirely subjective, and good choices will vary from one dataset
## to another. They just control which points get coloured
```
...
```r
bluefin_3 <- kin_power(bluefin_3, k = 0.5) ## important but boring internal calculation - just
## do it.
```

## Cross-contaminated and degraded samples

Now, we need to _try_ to find and remove cross-contaminated or
degraded samples. Checking for excess or deficiency of heterozygosity
is the way to go here, but there some subtle details to do with null
alleles and so on; see MS^[Which doesn't exist _yet_...].
...
## -0.11332 -0.10317 -0.07152  0.04287  0.81516 
## 
## Coefficients:
##              Estimate Std. Error t value Pr(>|t|)    
## (Intercept)  0.113322   0.004022   28.17   <2e-16 ***
## stagebefore -0.008192   0.005688   -1.44     0.15    
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## Residual standard error: 0.1554 on 2984 degrees of freedom
## Multiple R-squared:  0.0006946,	Adjusted R-squared:  0.0003597 
## F-statistic: 2.074 on 1 and 2984 DF,  p-value: 0.1499
```
...
```r
## note that the pvals _loci_ for the contaminated dataset are substantially
...
ctm_bluefin_4 <- kin_power(ctm_bluefin_3, k = 0.5) ## important but boring calculation - just
## run it
...
```r
## note the large additional bump around PLOD = 0, and lack of air-gap between HSPs and
## POPs/FSPs.
histoPLOD( hsps, log=FALSE, lb = 0, fullsib_cut = 100)
```
...
```r
## cross-contaminated samples are almost as clear
